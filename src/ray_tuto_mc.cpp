...
#pragma omp parallel for schedule(dynamic, 16)
	for(int py= 0; py < image.height(); py++)
	{
		...
		for(int px= 0; px < image.width(); px++)
		{
			...
			if(Hit hit= bvh.intersect(ray))
			{
				...
				UniformDirection directions(dirs, pn);			// genere des directions uniformes 1 / 2pi
				...
				Color pColor = mesh.mesh_material(mesh.materials()[hit.triangle_id]).diffuse;
				Color color = 1/(float)dirs * sum * pColor;
	...
	write_image(image, "render_rebond.png");
	write_image_hdr(image, "render_rebond.hdr");
